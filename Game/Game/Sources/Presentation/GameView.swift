...
private struct GuessConfirmationCard: View {
                ...
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    Button(action: confirmAction) {
                        HStack {
                            ...
                            Text(isSubmitting ? "Submitting\u{2026}" : "Confirm guess")
                                .font(.headline)
                                .fontWeight(.semibold)
                        }
    ...
    private var latitudeText: String {
        String(format: "%0.4f\u{00B0}", coordinates.latitude)
    }

    private var longitudeText: String {
        String(format: "%0.4f\u{00B0}", coordinates.longitude)
    }
...
private struct LookAroundPanel: View {
    ...
    var body: some View {
        VStack(spacing: isExpanded ? 16 : 0) {
            ...
            ZStack {
                if isLoading {
                    ProgressView("Loading scene\u{2026}")
